package muslimcompass.direction.finddirection.scheduler
...
import muslimcompass.direction.finddirection.helper.Constants.NAME_ID
import muslimcompass.direction.finddirection.helper.Constants.SUNSET
...
import muslimcompass.direction.finddirection.prayer.PrayTime
import muslimcompass.direction.finddirection.prayer.Prayers
...
class PrayAlarmReceiver : BroadcastReceiver() {
    ...
    val pendingFlags = if (SDK_INT >= 23) {
        PendingIntent.FLAG_CANCEL_CURRENT or PendingIntent.FLAG_IMMUTABLE
    } else {
        PendingIntent.FLAG_CANCEL_CURRENT
    }

    val mutablePendingFlags = if (SDK_INT >= 23) {
...
//                context.startService(service)
    ...
    fun setAlarm(context: Context) {
        ...
        val alarmIntent =
            PendingIntent.getBroadcast(context, Constants.ALARM_ID, intent, pendingFlags)
    ...
    fun cancelAlarm(context: Context) {
        ...
        val alarmIntent = if (SDK_INT >= Build.VERSION_CODES.M) {
            ...
            PendingIntent.getBroadcast(
                context,
                Constants.ALARM_ID, intent, PendingIntent.FLAG_UPDATE_CURRENT/*FLAG_CANCEL_CURRENT*/
            )
        }
        ...
        val locationListenerPendingIntent = if (SDK_INT >= Build.VERSION_CODES.M) {
            PendingIntent.getBroadcast(
                context,
                Constants.LOCATION_ID,
                passiveIntent,
            ...
            PendingIntent.getBroadcast(
                context,
                Constants.LOCATION_ID, passiveIntent, PendingIntent.FLAG_UPDATE_CURRENT/*FLAG_CANCEL_CURRENT*/
            )
        }
